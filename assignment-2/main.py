...
def setNext(triangle: Triangle):
    triangle.nextTri = findBest(triangle.adjTris)
    if triangle.nextTri is not None:
        triangle.nextTri.prevTri = triangle
        setNext(triangle.nextTri)
    else:
        triangle.prevTri = triangle
    return


def findBest(triangles: list[Triangle]) -> Union[Triangle, None]:
    best = None
    bestLength = 4
    bestNext = 4
    for tri in triangles:
        if tri.nextTri is None and tri.prevTri is None:
            lenNext = len(tri.adjTris)
            length = len(
                [adjacent for adjacent in tri.adjTris if adjacent.nextTri is None and adjacent.prevTri is None]
            )
            if length <= bestLength and lenNext <= bestNext:
                bestLength = length
                best = tri
                bestNext = lenNext
    return best
