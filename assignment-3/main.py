...
# vector functions add(), subtract(), scalarMult(), dotProduct(), crossProduct(), length(), normalize(), and
# triangleArea() provided
...
def buildTriangles(slice0: Slice, slice1: Slice):

    min_dist, min_indices = math.inf, (0, 0)

    for idx0, vert0 in enumerate(slice0.verts):
        for idx1, vert1 in enumerate(slice1.verts):
            dist = length(subtract(vert0.coords, vert1.coords))
            if dist < min_dist:
                min_dist = dist
                min_indices = (idx0, idx1)

    cyclic_slice0 = slice0.verts[min_indices[0]:] + slice0.verts[:min_indices[0]+1]
    cyclic_slice1 = slice1.verts[min_indices[1]:] + slice1.verts[:min_indices[1]+1]

    # initialize min area and direction arrays
    minArea = [[0 for i in range(len(slice0.verts)+1)] for i in range(len(slice1.verts)+1)]
    minDir = [[0 for i in range(len(slice0.verts)+1)] for i in range(len(slice1.verts)+1)]
    ...
    for idx in range(1, len(cyclic_slice0)):
        minArea[0][idx] = minArea[0][idx-1] + triangleArea(cyclic_slice0[idx-1].coords, cyclic_slice0[idx].coords, cyclic_slice1[0].coords)
        minDir[0][idx] = Dir.PREV_COL

    # Fill in col 0 of minArea and minDir, since it's a special case as there's no col -1
    for idx in range(1, len(cyclic_slice1)):
        minArea[idx][0] = minArea[idx-1][0] + triangleArea(cyclic_slice1[idx-1].coords, cyclic_slice1[idx].coords, cyclic_slice0[0].coords)
        minDir[idx][0] = Dir.PREV_ROW

    # Fill in the remaining entries of minArea and minDir
    for r in range(1, len(cyclic_slice1)):
        for c in range(1, len(cyclic_slice0)):
            area_left = minArea[r][c-1] + triangleArea(
                cyclic_slice0[c-1].coords, cyclic_slice0[c].coords, cyclic_slice1[r].coords)
            area_up = minArea[r-1][c] + triangleArea(
                cyclic_slice1[r-1].coords, cyclic_slice0[c].coords, cyclic_slice1[r].coords)
            (minArea[r][c], minDir[r][c]) = (area_left, Dir.PREV_COL) if area_left <= area_up else (area_up, Dir.PREV_ROW)
    ...
    c = len(cyclic_slice0)-1
    while True:
        if minDir[r][c] == Dir.PREV_ROW:
            triangles.append(Triangle([cyclic_slice1[r], cyclic_slice0[c], cyclic_slice1[r - 1]]))
            r = r - 1
        else:
            triangles.append(Triangle([cyclic_slice1[r], cyclic_slice0[c], cyclic_slice0[c - 1]]))
            c = c - 1
        if r == 0 and c == 0:
            break
...
def keyCallback(window, key, scancode, action, mods):
    ...
    if action == glfw.PRESS:
        ...
        if key == glfw.KEY_ESCAPE:  # quit upon E
            sys.exit(0)
